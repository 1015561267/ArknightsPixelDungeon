...
public class WndKeyBindings extends Window {
    ...
    public WndKeyBindings(Boolean controller) {
        ...
        LinkedHashMap<Integer, GameAction> defaults = controller ? PDAction.getControllerDefaults() : PDAction.getDefaults();
        ...
        RedButton btnDefaults = new RedButton(Messages.get(this, "default"), 9){
            ...
            protected void onClick() {
                changedBindings = controller ? PDAction.getControllerDefaults() : PDAction.getDefaults();
                for (BindingItem i : listItems){
                    int key1 = 0;
                    int key2 = 0;
                    int key3 = 0;
        ...
        private boolean onClick( float x, float y ){
            if (inside(x, y)){
                ...
                if (x >= this.x + 4*width()/5 - 1 && key2 != 0) {
                    ArknightsPixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, 3, key3, key1, key2));

                    //assigning second key
                } else if (x >= this.x + 3*width()/5 - 1 && key1 != 0) {
                    ArknightsPixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, 2, key2, key1, key3));

                    //assigning first key
                } else if (x >= this.x + 2*width()/5){
                    ArknightsPixelDungeon.scene().addToFront( new WndChangeBinding(gameAction, this, 1, key1, key2, key3));
            ...
            btnUnbind = new RedButton(Messages.get(this, "unbind"), 9){
                ...
                protected void onClick() {
                    if (action == GameAction.LEFT_CLICK && listItem.key2 == 0 && listItem.key3 == 0){
                        ArknightsPixelDungeon.scene().addToFront(new WndMessage(Messages.get(WndChangeBinding.class, "cant_unbind")));
                    } else {
