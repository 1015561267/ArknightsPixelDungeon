...
import com.unifier.arknightspixeldungeon.effects.CircleArc;
...
public class RadialMenu extends Window {
    ...
    public RadialMenu(String title, String desc, String[] optionTexts, Image[] optionIcons){
        ...
        int size = PDSettings.interfaceSize() == 0 ? 140 : 200;
        resize(Game.width, Game.height);

        slots = optionTexts.length;
        center = new PointF(width/2, height/2);
        int length = PDSettings.interfaceSize() == 0 ? 57 : 80;
        ...
        descTxt.maxWidth(PDSettings.interfaceSize() == 0 ? 80 : 100);
    ...
    private static Image getBGTexture(int size, boolean inner){
        if (size >= 200){
            if (!inner)  return new Image(Assets.RADIAL_MENU, 0, 0, 200, 200);
            else        return new Image(Assets.RADIAL_MENU, 340, 0, 120, 120);
        } else {
            if (!inner)  return new Image(Assets.RADIAL_MENU, 200, 0, 140, 140);
            else        return new Image(Assets.RADIAL_MENU, 340, 120, 90, 90);
        }
    }
