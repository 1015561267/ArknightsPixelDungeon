...
public class DriedRose extends Artifact {
	...
	public ArrayList<String> actions( Hero hero ) {
		...
		if (!FrostNovaQuestPlot.Quest.completed()){
		...
		if (action.equals(AC_SUMMON)) {
			...
			if (ghost != null)              GLog.i( Messages.get(this, "spawned") );
				...
				if (spawnPoints.size() > 0) {
					...
					if (!firstSummon) {
						ghost.yell( Messages.get(GhostHero.class, "hello", Dungeon.hero.operatorName()) );
	...
	@Override
	public String desc() {
		if (!FrostNovaQuestPlot.Quest.completed() && !isIdentified()){
