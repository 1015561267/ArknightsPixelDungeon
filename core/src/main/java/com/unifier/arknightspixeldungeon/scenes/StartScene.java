		...
		public void set( int slot ){
			...
			if (newGame){
				...
				if (hero == null){
					...
					classIcon = new Image(Icons.getDark(info.heroClass));
					add(classIcon);
					level = new BitmapText(PixelScene.pixelFont);
					add(level);
				} else {
					hero.copy(new Image(info.heroClass.spritesheet(), 0, 17*info.armorTier, 13, 17));
					
					classIcon.copy(Icons.getDark(info.heroClass));
				}
