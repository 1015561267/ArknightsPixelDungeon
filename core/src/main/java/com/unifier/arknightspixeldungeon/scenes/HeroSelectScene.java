...
import com.unifier.arknightspixeldungeon.windows.WndHeroInfo;
...
public class HeroSelectScene extends PixelScene {
    ...
    public void create() {
        ...
        infoButton = new IconButton(Icons.get(Icons.INFO)){
            ...
            protected void onClick() {
                ...
                ArknightsPixelDungeon.scene().addToFront(w);
            }
        ...
        btnOptions = new IconButton(Icons.get(Icons.SETTINGS)){
            @Override
            protected void onClick() {
                super.onClick();
        ...
        if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
            ...
            //ArknightsPixelDungeon.customSeed("");
        }
        ...
        add( btnExit );
        btnExit.visible = btnExit.active = !PDSettings.intro();
    ...
    private void updateOptionsColor(){
        //if (!ArknightsPixelDungeon.customSeed().isEmpty()){
        //    btnOptions.icon().hardlight(1f, 1.5f, 0.67f);
        //} else
        if (PDSettings.challenges() != 0){
    ...
    private void setSelectedHero(HeroClass cl){
        ...
        if (landscape()) {
            ...
            btnOptions.visible = btnOptions.active = !PDSettings.intro();
    ...
    private class HeroBtn extends StyledButton {
        ...
        HeroBtn ( HeroClass cl ){
            ...
            icon(new Image(cl.spritesheet(), 0, 102, 13, 17));
    ...
    private class GameOptions extends Component {
        ...
        protected void createChildren() {
            ...
            if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
                ...
                //FIXME for now seed run and daily seeed is banned as it's not the main point of today's focus,it will be considered later.
                /*StyledButton seedButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "custom_seed"), 6){
                    @Override
                    protected void onClick() {
                        String existingSeedtext = PDSettings.customSeed();
                        ArknightsPixelDungeon.scene().addToFront( new WndTextInput(Messages.get(HeroSelectScene.class, "custom_seed_title"),
                                ...
                                if (positive && seed != -1){
                                    ...
                                    for (GamesInProgress.Info info : GamesInProgress.checkAll()){
                                        if (info.customSeed.isEmpty() && info.seed == seed){
                                            PDSettings.customSeed("");
                                            icon.resetColor();
                                            ArknightsPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "custom_seed_duplicate")));
                                            return;
                                        }
                                    }

                                    PDSettings.customSeed(text);
                ...
                if (!PDSettings.customSeed().isEmpty()) seedButton.icon().hardlight(1f, 1.5f, 0.67f);;
                ...
                StyledButton dailyButton = new StyledButton(Chrome.Type.BLANK, Messages.get(HeroSelectScene.class, "daily"), 6){
                    ...
                    protected void onClick() {
                        ...
                        long diff = (PDSettings.lastDaily() + DAY) - Game.realTime;
                        if (diff > 24*HOUR){
                            ArknightsPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_unavailable_long", (diff / DAY)+1)));
                            return;
                        }

                        for (GamesInProgress.Info game : GamesInProgress.checkAll()){
                            if (game.daily){
                                ArknightsPixelDungeon.scene().addToFront(new WndMessage(Messages.get(HeroSelectScene.class, "daily_existing")));
                                return;
                            }
                        }
                        ...
                        ArknightsPixelDungeon.scene().addToFront(new WndOptions(
                                icon,
